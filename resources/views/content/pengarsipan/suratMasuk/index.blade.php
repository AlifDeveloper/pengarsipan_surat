...
@section('content')
    ...
    <!-- Search Section -->
    <form action="{{ auth()->user()->level_user == 'kades' ? route('pengarsipan-surat-masuk.index') : route('surat-masuk.index') }}" method="GET">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mt-4">
                ...
                <table class="min-w-full text-left table-auto  text-xs ">
                    <thead>
                        <tr>
                            ...
                            <th
                                class="p-2 w-14 text-center justify-center transition-colors cursor-pointer border border-slate-300 bg-blue-50 hover:bg-slate-100 {{ auth()->user()->level_user == 'kades' ? 'hidden' : '' }}">
                                <p class="flex items-center justify-between gap-2  font-medium leading-none ">
                                    Aksi
                                </p>
                            </th>
                    ...
                    <tbody>
                        @forelse ($surat_masuk as $index => $item)
                            <tr class="hover:bg-slate-50">
                                <td class="p-2 border border-slate-200 text-center">
                                    <p class="block  text-slate-800">
                                        {{ ($surat_masuk->currentPage() - 1) * $surat_masuk->perPage() + $loop->iteration }}
                                    </p>
                                </td>
                                ...
                                <td class="p-2 border flex border-slate-200 {{ auth()->user()->level_user == 'kades' ? 'hidden' : '' }}">
                                    <p class="block  text-red1">
                                        <a data-modal-toggle = "delete-modal"
                                            data-id = "{{ $item->no_suratmasuk }}"
                                            data-url = "{{ route('suratmasuk.delete', $item->no_suratmasuk) }}"
                                    ...
                                    <p class="block  text-red1">
                                        <a href="{{ route('surat-masuk.edit', $item->no_suratmasuk) }}"
                                            class=" flex bg-transparent text-white px-1 py-1 rounded-lg cursor-pointer  text-xs">
        ...
        <div class="flex justify-between items-center mt-4 text-blue">
            <a href="{{ $surat_masuk->previousPageUrl() }}"
                class="flex items-center justify-center rounded-md border border-blue py-2 px-3 text-center text-xs transition-all shadow-sm hover:shadow-lg hover:text-white hover:bg-blue hover:border-blue {{ $surat_masuk->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
            ...
            <div class="flex space-x-2">
                <div class="flex space-x-2">
                    @for ($i = 1; $i <= $surat_masuk->lastPage(); $i++)
                        <a href="{{ $surat_masuk->url($i) }}"
                        ...
                        {{ $surat_masuk->currentPage() == $i ? 'bg-blue text-white' : 'border border-slate-300' }}">
                            {{ $i }}
                        </a>
                    @endfor
                </div>
            </div>
            <a href="{{ $surat_masuk->nextPageUrl() }}"
                ...
                {{ $surat_masuk->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }}">
...
@section('scripts')
    ...
    <script>
        function resetForm() {
            ...
            window.location.href = '{{ auth()->user()->level_user == 'kades' ? route('pengarsipan-surat-masuk.index') : route('surat-masuk.index') }}';
        }
    </script>
@endsection
