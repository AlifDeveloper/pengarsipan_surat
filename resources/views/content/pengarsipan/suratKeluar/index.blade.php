...
@section('content')
    ...
    <!-- Search Section -->
    <form action="{{ auth()->user()->level_user == 'kades' ? route('pengarsipan-surat-keluar.index') : route('surat-keluar.index') }}" method="GET">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mt-4">
        <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
            ...
            <div class="ml-3 {{ auth()->user()->level_user == 'kades' ? 'hidden' : '' }}">
                <div class="relative">
                ...
                <table class="min-w-full text-left table-auto  text-xs ">
                    <thead>
                        <tr>
                            ...
                            <th
                                class="{{ auth()->user()->level_user == 'kades' ? 'hidden': '' }} p-2 w-14 text-center transition-colors cursor-pointer border border-slate-300 bg-blue-50 hover:bg-slate-100">
                                <p class="flex items-center justify-between gap-2  font-medium leading-none ">
                                    Aksi
                                </p>
                            </th>
                            ...
                            <tr class="hover:bg-slate-50">
                                <td class="p-2 border border-slate-200 text-center">
                                    <p class="block  text-slate-800">
                                        {{ ($surat_keluar->currentPage() - 1) * $surat_keluar->perPage() + $loop->iteration }}
                                    </p>
                                </td>
                                <td class="p-2 border border-slate-200">
                                    <p class="block  text-slate-800">
                                        {{ $item->kode_arsip }} [{{ $item->klasifikasi_arsip->nama_arsip }}]
                                    </p>
                                </td>
                                <td class="p-2 border border-slate-200">
                                    <p class="block  text-slate-800">
                                ...
                                <td class="p-2 border border-slate-200">
                                    <p class="block  text-slate-800">
                                        {{-- <a data-modal-toggle = "modal-detail" href="{{ asset('storage/' . $item->lampiran_pdf) }}" --}}
                                        <a data-modal-toggle = "modal-detail"
                                            data-file-pdf={{ asset('storage/' . $item->lampiran_pdf) }}
                                            data-file-gambar={{ asset('storage/' . $item->lampiran_gambar) }}
                                ...
                                {{-- modal delete --}}
                                <td class="p-2 border flex border-slate-200 {{ auth()->user()->level_user == 'kades' ? 'hidden' : '' }}">
                                    <p class="block  text-red1">
                                        <a data-modal-toggle = "delete-modal" data-id = "{{ $item->no_suratkeluar }}"
                                            data-url = "{{ route('surat-keluar.destroy', $item->no_suratkeluar) }}"
                                    ...
                                    <p class="block  text-red1">
                                        <a href="{{ route('surat-keluar.edit', $item->no_suratkeluar) }}"
                                            class=" flex bg-transparent text-white px-1 py-1 rounded-lg cursor-pointer  text-xs">
        ...
        <div class="flex justify-between items-center mt-4 text-blue">
            <a href="{{ $surat_keluar->previousPageUrl() }}"
                class="flex items-center justify-center rounded-md border border-blue py-2 px-3 text-center text-xs transition-all shadow-sm hover:shadow-lg hover:text-white hover:bg-blue hover:border-blue {{ $surat_keluar->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
            ...
            <div class="flex space-x-2">
                <div class="flex space-x-2">
                    @for ($i = 1; $i <= $surat_keluar->lastPage(); $i++)
                        <a href="{{ $surat_keluar->url($i) }}"
                        ...
                        {{ $surat_keluar->currentPage() == $i ? 'bg-blue text-white' : 'border border-slate-300' }}">
                            {{ $i }}
                        </a>
                    @endfor
                </div>
            </div>
            <a href="{{ $surat_keluar->nextPageUrl() }}"
                ...
                {{ $surat_keluar->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }}">
...
@section('scripts')
    ...
    <script>
        function resetForm() {
            ...
            window.location.href = '{{ auth()->user()->level_user == 'kades' ? route('pengarsipan-surat-keluar.index') : route('surat-keluar.index') }}';
        }
    </script>
@endsection
