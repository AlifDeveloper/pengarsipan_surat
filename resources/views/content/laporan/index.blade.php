...
<div class="flex items-center gap-6 mb-4">
    <div class="bg-white rounded-lg font-bold shadow-md mt-4 p-4 items-center justify-center text-center">
        <h1>{{ $totalSuratMasuk }}</h1>
        <h1>Surat Masuk  </h1>
    </div>
    <div class="bg-white rounded-lg font-bold shadow-md mt-4 p-4 items-center justify-center text-center">
        <h1>{{ $totalSuratKeluar }}</h1>
        <h1>Surat Keluar  </h1>
    </div>
</div>

<!-- Search Section -->
<form action="{{ auth()->user()->level_user == 'kades' ? route('laporan-surat.index') : route('laporan.index') }}" method="GET">
        ...
        <div class="flex flex-wrap">
            ...
            <div class="w-full md:w-4/12 mr-4">
                ...
                <select id="arsip" name="arsip" class="w-full text-sm bg-transparent border text-gray1 border-slate-200 rounded-md px-3 py-2">
                    ...
                    @foreach ( $klasifikasi_arsip as $arsip )
                        <option value="{{ $arsip->kode_arsip }}" {{ request('arsip') == $arsip->kode_arsip ? 'selected' : '' }}>{{ $arsip->kode_arsip }} [{{ $arsip->nama_arsip }}]</option>
                    @endforeach
                </select>
            </div>
...
<div class="bg-white p-6 rounded-lg shadow-md mt-4">
    <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
        <div class="flex items-center space-x-2">
            ...
            <div>
                ...
                <p class="text-slate-500 text-sm">Berikut adalah data surat masuk dan surat keluar periode {{ $namaBulanIndonesia }} {{ $tahun }}.</p>
            </div>
        </div>
        <div class="ml-3">
            <div class="relative">
    ...
    <div class="overflow-x-auto">
        <div class="border rounded-lg border-slate-50 overflow-hidden">
            <table class="min-w-full table-auto  text-xs ">
                ...
                <tbody>
                    @forelse ($dataSurat as $index => $item)
                        <tr class="hover:bg-slate-50">
                            <td class="p-2 border border-slate-200 text-center">
                                <p class="block  text-slate-800">
                                    {{ ($dataSurat->currentPage() - 1) * $dataSurat->perPage() + $loop->iteration }}
                                </p>
                            </td>
                            <td class="p-2 border border-slate-200">
                                <p class="block  text-slate-800">
                                    {{ $item->kode_arsip ? $item->kode_arsip . ' [' . optional($item->klasifikasi_arsip)->nama_arsip . ']' : '-' }}
                                </p>
                            </td>
                            <td class="p-2 border border-slate-200">
                                <p class="block  text-slate-800">
    ...
    <div class="flex justify-between items-center mt-4 text-blue">
        <a href="{{ $dataSurat->previousPageUrl() }}"
            class="flex items-center justify-center rounded-md border border-blue py-2 px-3 text-center text-xs transition-all shadow-sm hover:shadow-lg hover:text-white hover:bg-blue hover:border-blue {{ $dataSurat->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
        ...
        <div class="flex space-x-2">
            <div class="flex space-x-2">
                @for ($i = 1; $i <= $dataSurat->lastPage(); $i++)
                    <a href="{{ $dataSurat->url($i) }}"
                    ...
                    {{ $dataSurat->currentPage() == $i ? 'bg-blue text-white' : 'border border-slate-300' }}">
                        {{ $i }}
                    </a>
                @endfor
            </div>
        </div>
        <a href="{{ $dataSurat->nextPageUrl() }}"
            ...
            {{ $dataSurat->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }}">
...
<form id="cetakForm" method="GET" class="hidden">
    ...
    <input type="hidden" name="arsip" value="{{ request('arsip') }}">
...
@section('scripts')
    ...
    <script>
        function resetForm() {
            ...
            window.location.href = '{{ auth()->user()->level_user == 'kades' ? route('laporan-surat.index') : route('laporan.index') }}';
        }
    </script>
